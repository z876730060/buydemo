...
func UpdateContract(c *gin.Context) {
	...
	// Re-lookup party B name if changed
...
// SignContract signs a contract. If party_a signs, status → sent; if party_b signs, status → signed_by_b or completed
func SignContract(c *gin.Context) {
	...
	var req struct {
		Party     string `json:"party" binding:"required"` // a / b
		Signature string `json:"signature" binding:"required"` // base64 image data
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "数据格式错误: " + err.Error()})
		return
...
// GetContractParties returns options for creating contracts
func GetContractParties(c *gin.Context) {
	ptype := c.Query("type") // supplier / customer
	if ptype == "supplier" {
